...
public class Snake {
	...
	public void move() { //Moves the snake's head in the current direction and shifts the body segments to follow.
        ...
        double newY = segments.getFirst().getY() + deltaY; //current directions

        // Move body segments by updating their positions
        for (int i = segments.size() - 1; i > 0; i--) { //loop backwards to the part in front of it
            BodySegment current = segments.get(i); 
            BodySegment previous = segments.get(i - 1);
            current.setPosition(previous.getX(), previous.getY()); //move head to new pos to appear 'stretching'
        }
	...
	public boolean eatFood(Food f) {
        ...
        double distance = Math.sqrt(Math.pow(head.getX() - f.getX(), 2) + Math.pow(head.getY() - f.getY(), 2)); //calculates distance between head and food
        if (distance <= SEGMENT_SIZE) { // If the distance is less than or equal to the size of the food
            ...
            segments.add(new BodySegment(tail.getX(), tail.getY(), SEGMENT_SIZE));  //new bodysegment at exact location of tail
	 ...
	 */
	public boolean isInbounds() { //check if in bounds
		BodySegment head = segments.getFirst(); 
        return head.getX() >= 0 && head.getX() <= 1 && head.getY() >= 0 && head.getY() <= 1; //check if the head is in bounds of screen set
	}
